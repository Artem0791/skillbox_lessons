...
download = 0
percent = math.ceil(100 / time)
status = 0
for t in range(1, time + 1):
    if download <= size - speed:
        download += speed
        status += percent
        print('Прошло {} сек. Скачано {} из {} Мб ({}%)'.format(t, download, size, status))
    else:
        status += percent
        print('Прошло {} сек. Скачано {} из {} Мб ({}%)'.format(t, size, size, 100))
